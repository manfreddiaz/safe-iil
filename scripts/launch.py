...
EXPERIMENT_TEMPLATE = 'nohup python -m imitation.training --seed={seed} {setting} ' \
                      '{extra}'
SLEEP_TIME = 5  # in seconds

LOG_DIRECTORY = 'tails'
LOG_FILENAME_TEMPLATE = '{dir}/{setting}-{seed}.log'
...
def parse():
    ...
    parser.add_argument('setting', choices=['behavioral', 'default', 'safety'])
...
def call_module(setting, seed, arguments):
...
def call_multi_seed(setting, initial_seed, seeds, extra):
    process_handlers = []
...
def run_experiment(args):
    print('Experiments')
    print('===================================')
    print("Launching experiment <{experiment}> with <{setting}>.".format(experiment='run',
